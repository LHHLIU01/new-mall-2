  ...
  <div>
    ...
    <el-drawer  :visible.sync="show" :show-close="false" size='50%'>
      ...
      <el-row>
        ...
        <el-col :span="16">
          <template>
            <el-tag style="margin:5px 5px" type="success" v-for="(attr,index) in skuInfo.skuAttrValueList" :key="attr.id">{{attr.attrId}}-{{attr.valueId}}</el-tag>
          </template>
        </el-col>
      </el-row>
      <el-row>
...
export default {
    ...
    methods: {
      async getSkuList(pages = 1){
        ...
        let reuslt = await this.$API.sku.reqSkuList(page,limit)
        console.log(reuslt);
        if (reuslt.code==200) {
          this.total = reuslt.data.total
          this.records = reuslt.data.records
        }
      },
